...
when matches and literals alternate.
 ...
 chunk:
  ...
  1 byte : (optional) continuation of literal len, 0 - 240 ($F0)
 ...
 */
size_t findLongestMatch(const uint8_t* matchPtr,
						const uint8_t* inBuf,
						size_t inLen,
						size_t* pMatchOffset)
{
	...
	for (size_t ii = 0; ii < maxStartOffset; ii++) {
		...
		//size_t maxMatchLen = inLen - maxStartOffset;
 ...
 */
size_t compressBufferOptimally(uint8_t* outBuf,
							   const uint8_t* inBuf,
							   size_t inLen)
{
	...
	for (unsigned int i = inLen - 1; i < inLen; i--) {
		...
		// length.
		size_t matchOffset;
		size_t longestMatch = findLongestMatch(inBuf + i,
											   inBuf,
											   inLen,
											   &matchOffset);
 ...
 * Returns the amount of data in "outBuf" on success, or 0 on failure.
 */
size_t compressBufferGreedily(uint8_t* outBuf,
							  const uint8_t* inBuf,
							  size_t inLen)
{
	const uint8_t* inPtr = inBuf;
	...
	*outPtr++ = LZ4FH_MAGIC;
	...
	while (inPtr < inBuf + inLen) {
		...
		size_t matchOffset;
		size_t longestMatch = findLongestMatch(inPtr,
											   inBuf,
											   inLen,
											   &matchOffset);
		if (longestMatch < MIN_MATCH_LEN) {
	...
	*outPtr++ = EOD_MATCH_TOKEN;
	...
	return outPtr - outBuf;			// length of output
}
...
size_t uncompressBuffer(uint8_t* outBuf, const uint8_t* inBuf, size_t inLen)
{
	...
	if (inPtr - inBuf != (long) inLen) {
		fprintf(stderr, "Warning: uncompress used only %ld of %ld bytes\n",
				(long)(inPtr - inBuf), inLen);
	}
 ...
 */
int compressFile(const char* outFileName,
				 const char* inFileName,
				 bool doPreserveHoles,
...
int main(int argc, char* argv[])
{
	...
	if (mode == MODE_COMPRESS) {
		...
		result = compressFile(outFileName,
							  inFileName,
							  doPreserveHoles,
							  useGreedyParsing);
	}
